        ...
        C: GenericConfig<D, F = F>,
{
    ...
    let mut trace_commitments = timed!(
        timing,
        ...
        trace_poly_values
            ...
            .map(|(trace, table)| {
                timed!(
                    ...
                    {
                        println!("trace len: {}, values len: {}", trace.len(), trace[0].values.len());
                        let ret = {
                            let trace_flatten = &trace.iter().flat_map(|p|p.values.to_vec()).collect::<Vec<_>>();
                            let poly = PolynomialBatch::from_values_with_gpu(
                                trace_flatten,
                                trace.len(),
                                trace[0].values.len(),
                                rate_bits,
    ...
    S: Stark<F, D>,
{
    ...
    println!("aux len: {}, values len: {}", auxiliary_polys.len(), auxiliary_polys[0].values.len());
    ...
    println!("quotient len: {}, values len: {}", all_quotient_chunks.len(), all_quotient_chunks[0].coeffs.len());
        ...
        S: Stark<F, D>,
{
    ...
    let auxiliary_polys_commitment = timed!(
        ...
        "compute auxiliary polynomials commitment",
        {
            ...
            {
                let auxiliary_polys_flatten = &auxiliary_polys.iter().flat_map(|p|p.values.to_vec()).collect::<Vec<_>>();
