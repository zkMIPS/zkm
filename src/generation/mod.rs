...
pub fn generate_traces<F: RichField + Extendable<D>, const D: usize>(
    ...
    let mut state = GenerationState::<F>::new(inputs.clone(), &KERNEL.code, 4000000).unwrap();
...
fn simulate_cpu<F: RichField + Extendable<D>, const D: usize>(
    ...
    loop {
        ...
        let halt = state.registers.is_kernel && (pc == 0xa7ef00d0 || step == state.step || state.registers.exited);
        log::debug!("pc: {:X}", pc);
        if halt {
