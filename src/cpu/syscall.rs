...
pub fn eval_packed<P: PackedField>(
    ...
    //3 a0 is zero
    //4 v0 value is right
    yield_constr.constraint(filter * is_SYSMMAP * is_a0_zero * (v0_in_a0_zero - result_v0));
    //check:
    //1 is_syscall
    //2 sysnum==SYSMMAP
    //3 a0 is not zero
    //4 v0 value is right
    yield_constr.constraint(filter * is_SYSMMAP * (P::ONES - is_a0_zero) * (a0 - result_v0));
...
pub fn eval_ext_circuit<F: RichField + Extendable<D>, const D: usize>(
    ...
    let result_v0 = lv.mem_channels[4].value[0];
    let result_v1 = lv.mem_channels[5].value[0];
    let filter_0 = builder.mul_extension(filter, is_SYSMMAP);
