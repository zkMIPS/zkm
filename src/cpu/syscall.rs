...
pub fn eval_packed<P: PackedField>(
    ...
    yield_constr.constraint(filter * is_a0_zero * (v0_in_a0_zero - result_v0));
    yield_constr.constraint(filter * (P::ONES - is_a0_zero) * (a0 - result_v0));
    ...
    yield_constr.constraint(filter * a0_is_not_FD_STDIN * (v0_in_a0_is_not_FD_STDIN - result_v0));
    yield_constr.constraint(filter * a0_is_not_FD_STDIN * (v1_in_a0_is_not_FD_STDIN - result_v1));
    yield_constr.constraint(filter * a0_is_FD_STDIN * (v0 - result_v0));
    yield_constr.constraint(filter * a0_is_FD_STDIN * (v1 - result_v1));
    ...
    yield_constr.constraint(filter * a0_is_FD_STDIN * (v0_in_a0_is_FD_STDIN - result_v0));

    yield_constr.constraint(filter * a0_is_FD_STDIN * (v1 - result_v1));

    yield_constr.constraint(filter * a0_is_FD_STDOUT_or_FD_STDERR * (P::ONES - result_v0));

    yield_constr.constraint(filter * a0_is_FD_STDOUT_or_FD_STDERR * (v1 - result_v1));

    yield_constr.constraint(
        filter * a0_is_else * (v0_in_a0_is_not_FD_STDOUT_and_FD_STDERR_and_FD_STDIN - result_v0),
    );

    yield_constr.constraint(
        filter * a0_is_else * (v1_in_a0_is_not_FD_STDIN_and_FD_STDERR_and_FD_STDIN - result_v1),
    );
}
