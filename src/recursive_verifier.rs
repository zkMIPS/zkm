...
use crate::proof::{
    AllProof, AllProofChallenges, StarkOpeningSet, StarkProofChallenges,
};
...
where
{
    ...
    let degree_bits = proof_with_pis.stark_proofs.recover_degree_bits(inner_config);
...
>(
    builder: &mut CircuitBuilder<F, D>,
    stark: AllStark<F, D>,
    proof_with_pis: StarkProofTarget<D>,
...
>(
    ...
    let public_inputs = builder.add_virtual_targets(PUBLIC_INPUTS);
...
pub fn add_virtual_stark_proof<F: RichField + Extendable<D>, S: Stark<F, D>, const D: usize>(
    ...
    let num_leaves_per_oracle = once(S::COLUMNS)
        .chain(
            stark
                .uses_lookups()
                .then(|| stark.num_lookup_helper_columns(config)),
        )
    ...
    StarkProofTarget {
        ...
        auxiliary_polys_cap: permutation_zs_cap,
    ...
    W: Witness<F>,
{
    ...
    if let (permutation_zs_cap_target, permutation_zs_cap) =
        (&proof_target.auxiliary_polys_cap, &proof.auxiliary_polys_cap)
    {
