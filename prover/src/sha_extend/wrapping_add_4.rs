...
impl <F: Field> WrappingAdd4Op<F> {
    pub fn generate_trace(&mut self, a: [u8; 4], b: [u8; 4], c: [u8; 4], d: [u8; 4]) -> u32{
        let base = 256;
        ...
        a_u32.wrapping_add(b_u32).wrapping_add(c_u32).wrapping_add(d_u32)
    }
}
...
pub(crate) fn wrapping_add_packed_constraints<P: PackedField>(
    ...
    for i in 0..4 {
        ...
        result.push(cols.is_carry_0[i] + cols.is_carry_1[i] + cols.is_carry_2[i] + cols.is_carry_3[i] - P::ONES);
    }
...
>(
    ...
    for i in 0..4 {
        ...
        let tmp4 = builder.add_extension(tmp, tmp2);
        let tmp5 = builder.add_extension(tmp4, tmp3);
    ...
    for i in 0..4 {
        ...
        let mut overflow = builder.sub_extension(tmp3, cols.value[i]);
        if i > 0 {
            overflow = builder.add_extension(overflow, cols.carry[i - 1]);
        }
        let tmp4 = builder.mul_extension(cols.carry[i], base);
        result.push(builder.sub_extension(tmp4, overflow));
    }
