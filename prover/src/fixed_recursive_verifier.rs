...
#[cfg(feature = "gpu")]
use crate::prover::{prove_gpu, prove_with_assumptions_gpu};
    ...
    C::Hasher: AlgebraicHasher<F>,
{
    ...
    pub fn prove_root_gpu(
        ...
        // let all_proof = prove::<F, C, D>(all_stark, kernel, config, timing)?;
        let all_proof = prove_gpu::<F, C, D>(all_stark, kernel, config, timing, ctx)?;
        verify_proof(all_stark, all_proof.clone(), config).unwrap();
        let mut root_inputs = PartialWitness::new();
    ...
    #[cfg(feature = "gpu")]
    pub fn prove_root_with_assumption_gpu(
        &self,
        all_stark: &AllStark<F, D>,
        kernel: &Kernel,
        config: &StarkConfig,
        ...
        let (all_proof, receipts) = prove_with_assumptions_gpu::<F, C, D>(
            all_stark,
            kernel,
            config,
            timing,
